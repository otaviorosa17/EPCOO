...
class Enemy1 extends Enemy{
	...
	Enemy1(long currentTime){
		...
		super.setRadius(9.0);
		super.setNextEnemy(currentTime + 2000);
	}
	...
	public double[] getExplosion_start() {
		return super.getExplosion_start();
	}
	public double[] getExplosion_end() {
		return super.getExplosion_end();
	}
	public double getRadius() {
		return super.getRadius();
	}
	public long getNextEnemy() {
		return super.getNextEnemy();
	}
...
class Enemy2 extends Enemy{
	...
	Enemy2(long currentTime) {
		...
		super.setRadius(12.0);
		super.setNextEnemy(currentTime);
		count++;
	}
	...
	public void throwNew(long currentTime) {
		if(currentTime > super.getNextEnemy()){
			...
			int free = Main.findFreeIndex(super.getStates());
							
			if(free < super.getStates().length){
				
				super.getX()[free] = this.getSpawnX();
				super.getY()[free] = -10.0;
				super.getV()[free] = 0.42;
				super.getAngle()[free] = (3 * Math.PI) / 2;
				super.getRV()[free] = 0.0;
				super.getStates()[free] = Main.ACTIVE;

				count++;
				
				if(Enemy2.getCount() < 10){
					
					super.setNextEnemy(currentTime + 120);
				}
				else {
					count = 0;
					spawnX = Math.random() > 0.5 ? GameLib.WIDTH * 0.2 : GameLib.WIDTH * 0.8;
					super.setNextEnemy((long) (currentTime + 3000 + Math.random() * 3000));
				}
			}
		}
	}
